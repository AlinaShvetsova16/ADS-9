...
std::vector<char> getPerm2(const PMTree& tree, int num) {
    ...
    while (remaining > 0) {
        ...
        int count = 0;
        for (const auto* child : current->children) {
            if (std::find(available.begin(), available.end(), child->value) != available.end()) {
        ...
        available.erase(std::remove(available.begin(), available.end(), nextNode->value), available.end());
